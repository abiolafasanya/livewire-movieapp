<div class="flex flex-col md:flex-row items-center" x-data="{ isOpen: true }" @click.outside="isOpen = false">
    <div class="relative md:mt-0 mt-2">
        ...
        wire:model.debounce.500ms="search" type="text"
         ...
         @keydown.tab="isOpen = false"
            class="bg-gray-800 pl-10 text-sm focus:border-gray-500 focus:outline-none rounded-full w-64 px-4 py-2"
            placeholder="Search">
        <div class="absolute top-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="fill-current w-5 mt-2 ml-2 text-gray-500" fill="#FFF">
    ...
    <div class="md:ml-4 md:mt-0 mt-2">
        <a href="#">
            <img src="https://www.placecage.com/200/300" alt="avatar" class="rounded-full w-8 h-8">
        </a>
    </div>

    @if (strlen($search) > 2)
        <div class="absolute bg-gray-800 rounded w-64 md:mt-12 mt-24 top-4 text-sm" 
                    ...
                    <ul>
                        <li class="border-b border-gray-700">
                            <a href="{{ route('movie', $result['id']) }}"
                                class="hover:bg-gray-700 px-3 py-3 flex items-center space-x-4 "
